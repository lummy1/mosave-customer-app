        ...
        <Public>
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={onSubmit}>
                <div>
                    ...
                    <input type="email" name="email" id="email" autoComplete='off' value={email} onFocus={onFocus} onChange={onChange} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required={true} />
                    <small className="mt-2 text-xs text-red-600 dark:text-red-600">{touched.email && formErrors.email}</small>
                    </div>
                <div>
                    <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                    <div className="relative">
                        <input type={passwordType} autoComplete='off' name="password" id="password" onFocus={onFocus} value={password} onChange={onChange} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required={true} />
                        <div className="absolute inset-y-0 right-0 flex items-center pr-2.5">
                            <button type='button' onClick={() => setPasswordType(passwordType === "password" ? 'text' : 'password')}>
                                {passwordType === "password" ? <FaEyeSlash className="dark:text-white" /> : <FaEye className="dark:text-white" />}
                            </button>
                        </div>
                    </div>
                    <small className="mt-2 text-xs text-red-600 dark:text-red-600">{touched.password && formErrors.password}</small>
                </div>
